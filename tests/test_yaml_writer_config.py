"""Test the YamlWriterConfig stuff."""
from yamkix.yaml_writer_config import YamlWriterConfig
from yamkix.yaml_writer_config import get_opinionated_yaml_writer
from ruamel.yaml import YAML


def test_default_values():
    """Test YamlWriterConfig default values."""
...
def test_get_opinionated_yaml_writer_with_defaults():
    ...
    sut: YAML = get_opinionated_yaml_writer(YamlWriterConfig())
    assert sut.typ == ["rt"]
    assert sut.explicit_start
    assert not sut.explicit_end
    assert not sut.default_flow_style
